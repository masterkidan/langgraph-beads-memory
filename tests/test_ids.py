...
def test_derive_fact_id_is_deterministic():
    a = derive_fact_id(NS, "passive_capture", "msg-1", "hello world")
    b = derive_fact_id(NS, "passive_capture", "msg-1", "hello world")
    assert isinstance(a, uuid.UUID) and a == b


def test_derive_fact_id_varies_by_inputs():
    base = derive_fact_id(NS, "passive_capture", "msg-1", "hello")
    assert base != derive_fact_id(NS, "passive_capture", "msg-2", "hello")
    assert base != derive_fact_id(NS, "passive_capture", "msg-1", "bye")
    assert base != derive_fact_id(uuid.uuid4(), "passive_capture", "msg-1", "hello")


def test_short_id_format():
    fid = derive_fact_id(NS, "passive_capture", "msg-1", "hello")
    s = short_id(fid)
